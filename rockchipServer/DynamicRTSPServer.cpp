...
static ServerMediaSession *createNewSMS(UsageEnvironment &env, char const *fileName) {
    ...
    if (strcmp(fileName, "live") == 0) {
        ...
        sms->addSubsession(CustomMediaSubsession::createNew(env, fileName, reuseSource));
        ...
        sms->addSubsession(
            H265VideoFileServerMediaSubsession::createNew(env, fileName, reuseSource));
    }
    return sms;
}
