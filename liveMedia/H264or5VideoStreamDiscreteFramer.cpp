...
H264or5VideoStreamDiscreteFramer ::H264or5VideoStreamDiscreteFramer(
    int hNumber, UsageEnvironment &env, FramedSource *inputSource, Boolean includeStartCodeInOutput,
...
void H264or5VideoStreamDiscreteFramer::doGetNextFrame() {
    ...
    if (fInsertAccessUnitDelimiters && pictureEndMarker()) {
        ...
        if (fLastSeenVPSSize > 0) {
            ...
            memmove(fTo, fLastSeenVPS, fLastSeenVPSSize);
            fTo += fLastSeenVPSSize;
        }
        if (fLastSeenSPSSize > 0) {
            ...
            memmove(fTo, fLastSeenSPS, fLastSeenSPSSize);
            fTo += fLastSeenSPSSize;
        }
        if (fLastSeenPPSSize > 0) {
            ...
            memmove(fTo, fLastSeenPPS, fLastSeenPPSSize);
            fTo += fLastSeenPPSSize;
        }
...
void H264or5VideoStreamDiscreteFramer ::afterGettingFrame(void *clientData, unsigned frameSize,
