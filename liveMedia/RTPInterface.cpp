...
public:
    tcpStreamRecord(int streamSocketNum, unsigned char streamChannelId, tcpStreamRecord *next);
...
void RTPInterface::setServerRequestAlternativeByteHandler(
    UsageEnvironment &env, int socketNum, ServerRequestAlternativeByteHandler *handler,
    void *clientData) {
...
Boolean RTPInterface::sendPacket(unsigned char *packet, unsigned packetSize) {
    ...
    for (tcpStreamRecord *stream = fTCPStreams; stream != NULL; stream = nextStream) {
        ...
        if (!sendRTPorRTCPPacketOverTCP(packet, packetSize, stream->fStreamSocketNum,
...
Boolean RTPInterface::handleRead(unsigned char *buffer, unsigned bufferMaxSize, unsigned &bytesRead,
    ...
    if (fNextTCPReadStreamSocketNum < 0) {
        ...
        int curBytesRead;
        while ((curBytesRead = readSocket(envir(), fNextTCPReadStreamSocketNum, &buffer[bytesRead],
                                          curBytesToRead, fromAddress)) > 0) {
...
#ifdef DEBUG_SEND
                fprintf(stderr,
                        "sendDataOverTCP: blocking send() failed (delivering %d bytes out of %d); "
                        "closing socket %d\n",
...
void SocketDescriptor::registerRTPInterface(unsigned char streamChannelId,
    ...
    fprintf(
        stderr,
        "SocketDescriptor(socket %d)::registerRTPInterface(channel %d): isFirstRegistration %d\n",
    ...
    if (isFirstRegistration) {
        ...
        fEnv.taskScheduler().setBackgroundHandling(
            fOurSocketNum, SOCKET_READABLE | SOCKET_EXCEPTION, handler, this);
    }
}
...
void SocketDescriptor ::deregisterRTPInterface(unsigned char streamChannelId) {
    ...
    if (fSubChannelHashTable->IsEmpty()) {
        ...
        if (fAreInReadHandlerLoop) {
            fDeleteMyselfNext =
                True;  // we can't delete ourself yet, but we'll do so from "tcpReadHandler()" below
        } else {
            delete this;
        }
    }
...
void SocketDescriptor::tcpReadHandler(SocketDescriptor *socketDescriptor, int mask) {
    ...
    while (!socketDescriptor->fDeleteMyselfNext && socketDescriptor->tcpReadHandler1(mask) &&
           --count > 0) {
    }
...
Boolean SocketDescriptor::tcpReadHandler1(int mask) {
...
#ifdef DEBUG_RECEIVE
            fprintf(stderr,
                    "SocketDescriptor(socket %d)::tcpReadHandler(): readSocket(1 byte) returned %d "
                    "(error)\n",
    ...
    switch (fTCPReadingState) {
...
#ifdef DEBUG_RECEIVE
                fprintf(stderr,
                        "SocketDescriptor(socket %d)::tcpReadHandler(): Saw nonexistent stream "
                        "channel id: 0x%02x\n",
                        fOurSocketNum, c);
#endif
                fTCPReadingState = AWAITING_DOLLAR;
        ...
        case AWAITING_PACKET_DATA: {
            callAgain = False;
            fTCPReadingState =
                AWAITING_DOLLAR;  // the next state, unless we end up having to read more data in the current state
            ...
            if (rtpInterface != NULL) {
...
#ifdef DEBUG_RECEIVE
                    fprintf(stderr,
                            "SocketDescriptor(socket %d)::tcpReadHandler(): reading %d bytes on "
                            "channel %d\n",
...
#ifdef DEBUG_RECEIVE
                    fprintf(stderr,
                            "SocketDescriptor(socket %d)::tcpReadHandler(): No handler proc for "
                            "\"rtpInterface\" for channel %d; need to skip %d remaining bytes\n",
...
#ifdef DEBUG_RECEIVE
                        fprintf(stderr,
                                "SocketDescriptor(socket %d)::tcpReadHandler(): readSocket(1 byte) "
                                "returned %d (error)\n",
                                fOurSocketNum, result);
#endif
                        fReadErrorOccurred = True;
                ...
                fprintf(stderr,
                        "SocketDescriptor(socket %d)::tcpReadHandler(): No \"rtpInterface\" for "
                        "channel %d\n",
...
tcpStreamRecord ::tcpStreamRecord(int streamSocketNum, unsigned char streamChannelId,
