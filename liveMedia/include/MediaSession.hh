...
public:
    ...
    Boolean initiateByMediaType(char const *mimeType, MediaSubsession *&resultSubsession,
...
protected:
    ...
    static char *lookupPayloadFormat(unsigned char rtpPayloadType, unsigned &rtpTimestampFrequency,
...
public:
    ...
    void receiveRawMP3ADUs() {
        fReceiveRawMP3ADUs = True;
    }  // optional hack for audio/MPA-ROBUST; must not be called after initiate()
    char *&connectionEndpointName() { return fConnectionEndpointName; }
    char const *connectionEndpointName() const { return fConnectionEndpointName; }
    ...
    unsigned attrVal_unsigned(char const *attrName) const {
        return (unsigned)attrVal_int(attrName);
    }
    ...
    struct {
        ...
        u_int32_t timestamp;
        Boolean
            infoIsNew;  // not part of the RTSP header; instead, set whenever this struct is filled in
    } rtpInfo;
    ...
    SRTPCryptographicContext *getCrypto() const {
        return fCrypto != NULL ? fCrypto : fParent.getCrypto();
    }
protected:
    ...
    void setAttribute(char const *name, char const *value = NULL,
...
protected:
    ...
    HashTable
        *fAttributeTable;  // for "a=fmtp:" attributes.  (Later an array by payload type #####)

    // Fields set or used by initiate():
    Groupsock *fRTPSocket;
    Groupsock *fRTCPSocket;  // works even for unicast
