...
public:
    ProxyRTSPClient(class ProxyServerMediaSession &ourServerMediaSession, char const *rtspURL,
                    char const *username, char const *password, portNumBits tunnelOverHTTPPortNum,
...
typedef ProxyRTSPClient *createNewProxyRTSPClientFunc(
    ProxyServerMediaSession &ourServerMediaSession, char const *rtspURL, char const *username,
    char const *password, portNumBits tunnelOverHTTPPortNum, int verbosityLevel,
...
public:
    static ProxyServerMediaSession *createNew(
        ...
        char const *streamName = NULL, char const *username = NULL, char const *password = NULL,
        portNumBits tunnelOverHTTPPortNum = 0,
        // for streaming the *proxied* (i.e., back-end) stream
        int verbosityLevel = 0, int socketNumToServer = -1,
...
protected:
    ProxyServerMediaSession(UsageEnvironment &env, GenericMediaServer *ourMediaServer,
                            ...
                            int socketNumToServer, MediaTranscodingTable *transcodingTable,
...
private:
    ...
    PresentationTimeSubsessionNormalizer(PresentationTimeSessionNormalizer &parent,
...
public:
    ...
    PresentationTimeSubsessionNormalizer *createNewPresentationTimeSubsessionNormalizer(
...
private:  // called only from within "~PresentationTimeSubsessionNormalizer":
    ...
    void removePresentationTimeSubsessionNormalizer(
...
private:
    ...
    PresentationTimeSubsessionNormalizer
        *fMasterSSNormalizer;  // used for subsessions that have been RTCP-synced

    struct timeval
        fPTAdjustment;  // Added to (RTCP-synced) subsession presentation times to 'normalize' them with wall-clock time.
};
