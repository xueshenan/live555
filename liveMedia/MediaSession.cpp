...
Boolean MediaSession::lookupByName(UsageEnvironment &env, char const *instanceName,
...
MediaSession::MediaSession(UsageEnvironment &env)
    : Medium(env),
      ...
      fMaxPlayStartTime(0.0f),
      fMaxPlayEndTime(0.0f),
      fAbsStartTime(NULL),
      fAbsEndTime(NULL),
      fScale(1.0f),
      fSpeed(1.0f),
      fMediaSessionType(NULL),
      fSessionName(NULL),
      fSessionDescription(NULL),
      fControlPath(NULL),
      fMIKEYState(NULL),
      fCrypto(NULL) {
...
Boolean MediaSession::initializeWithSDP(char const *sdpDescription) {
    ...
    while (sdpLine != NULL) {
        ...
        if ((sscanf(sdpLine, "m=%s %hu RTP/AVP %u", mediumName, &subsession->fClientPortNum,
                    &payloadFormat) == 3 ||
             sscanf(sdpLine, "m=%s %hu/%*u RTP/AVP %u", mediumName, &subsession->fClientPortNum,
                    &payloadFormat) == 3) &&
            payloadFormat <= 127) {
            protocolName = "RTP";
        } else if ((sscanf(sdpLine, "m=%s %hu RTP/SAVP %u", mediumName, &subsession->fClientPortNum,
                           &payloadFormat) == 3 ||
                    sscanf(sdpLine, "m=%s %hu/%*u RTP/SAVP %u", mediumName,
                           &subsession->fClientPortNum, &payloadFormat) == 3) &&
                   payloadFormat <= 127) {
            protocolName = "SRTP";
        } else if ((sscanf(sdpLine, "m=%s %hu UDP %u", mediumName, &subsession->fClientPortNum,
                           &payloadFormat) == 3 ||
                    sscanf(sdpLine, "m=%s %hu udp %u", mediumName, &subsession->fClientPortNum,
                           &payloadFormat) == 3 ||
                    sscanf(sdpLine, "m=%s %hu RAW/RAW/UDP %u", mediumName,
                           &subsession->fClientPortNum, &payloadFormat) == 3) &&
                   payloadFormat <= 127) {
        ...
        if (subsession->fCodecName == NULL) {
            subsession->fCodecName =
                lookupPayloadFormat(subsession->fRTPPayloadFormat,
                                    subsession->fRTPTimestampFrequency, subsession->fNumChannels);
            if (subsession->fCodecName == NULL) {
                char typeStr[20];
        ...
        if (subsession->fRTPTimestampFrequency == 0) {
            subsession->fRTPTimestampFrequency =
                guessRTPTimestampFrequency(subsession->fMediumName, subsession->fCodecName);
        }
    }
...
static Boolean parseTwoStringValues(char const *sdpLine, char const *searchFormat, char *&result1,
                                    char *&result2) {
    Boolean parseSuccess = False;
    ...
    if (sscanf(sdpLine, searchFormat, buffer1, buffer2) == 2) {
        delete[] result1;
        result1 = strDup(buffer1);
        delete[] result2;
        result2 = strDup(buffer2);
        parseSuccess = True;
    }
    delete[] buffer1;
    delete[] buffer2;
...
static Boolean parseSourceFilterAttribute(char const *sdpLine,
    ...
    do {
        if (!(parseStringValue(sdpLine, "a=source-filter: incl IN IP4 %*s %s", sourceName) ||
              parseStringValue(sdpLine, "a=source-filter: incl IN IP6 %*s %s", sourceName)))
            break;
...
char *MediaSession::lookupPayloadFormat(unsigned char rtpPayloadType, unsigned &freq,
                                        unsigned &nCh) {
...
unsigned MediaSession::guessRTPTimestampFrequency(char const *mediumName, char const *codecName) {
    ...
    if (strcmp(codecName, "MPA") == 0 || strcmp(codecName, "MPA-ROBUST") == 0 ||
...
Boolean MediaSession ::initiateByMediaType(char const *mimeType, MediaSubsession *&resultSubsession,
...
MediaSubsession::MediaSubsession(MediaSession &parent)
    : serverPortNum(0),
      ...
      fConnectionEndpointName(NULL),
      fClientPortNum(0),
      fRTPPayloadFormat(0xFF),
      fSavedSDPLines(NULL),
      fMediumName(NULL),
      fCodecName(NULL),
      fProtocolName(NULL),
      fRTPTimestampFrequency(0),
    ...
    do {
        ...
        if (fClientPortNum != 0 && (honorSDPPortChoice || IsMulticastAddress(tempAddr))) {
            ...
            while (1) {
                ...
                if ((fClientPortNum & 1) != 0) {  // it's odd
                    ...
                    Groupsock *existing =
                        (Groupsock *)socketHashTable->Add((char const *)key, fRTPSocket);
        ...
        if (fRTPSource != NULL && fRTCPSocket != NULL) {
            ...
            fRTCPInstance = RTCPInstance::createNew(
                env(), fRTCPSocket, totSessionBandwidth, (unsigned char const *)fParent.CNAME(),
                NULL /* we're a client */, fRTPSource, False /* we're not a data transmitter */,
                ourCrypto);
...
double MediaSubsession::getNormalPlayTime(struct timeval const &presentationTime) {
    if (rtpSource() == NULL || rtpSource()->timestampFrequency() == 0)
        return 0.0;  // no RTP source, or bad freq!
    ...
    if (!rtpSource()->hasBeenSynchronizedUsingRTCP()) {
        ...
        if (rtpInfo.infoIsNew) {
            ...
            if (fNPT_PTS_Offset == 0.0)
                return 0.0;  // error: The "rtpInfo" structure was apparently never filled in
            return (double)(ptsDouble * scale() + fNPT_PTS_Offset);
        }
    }
}

void MediaSubsession ::setAttribute(char const *name, char const *value,
                                    Boolean valueIsHexadecimal) {
...
Boolean MediaSubsession::parseSDPAttribute_rtpmap(char const *sdpLine) {
    ...
    if (sscanf(sdpLine, "a=rtpmap: %u %[^/]/%u/%u", &rtpmapPayloadFormat, codecName,
               &rtpTimestampFrequency, &numChannels) == 4 ||
        sscanf(sdpLine, "a=rtpmap: %u %[^/]/%u", &rtpmapPayloadFormat, codecName,
               &rtpTimestampFrequency) == 3 ||
    ...
    do {
        ...
        if (strcmp(fProtocolName, "UDP") == 0) {
            ...
            Boolean doNormalMBitRule =
                False;  // default behavior if "createSimpleRTPSource" is True
            if (strcmp(fCodecName, "QCELP") == 0) {  // QCELP audio
                fReadSource = QCELPAudioRTPSource::createNew(
                    env(), fRTPSocket, fRTPSource, fRTPPayloadFormat, fRTPTimestampFrequency);
                ...
                fReadSource = fRTPSource = MPEG4GenericRTPSource::createNew(
                    env(), fRTPSocket, fRTPPayloadFormat, fRTPTimestampFrequency, fMediumName,
                    attrVal_strToLower("mode"), attrVal_unsigned("sizelength"),
                    attrVal_unsigned("indexlength"), attrVal_unsigned("indexdeltalength"));
                ...
                fReadSource = fRTPSource = H265VideoRTPSource::createNew(
                    env(), fRTPSocket, fRTPPayloadFormat, expectDONFields, fRTPTimestampFrequency);
                       ...
                       || strcmp(fCodecName, "VND.ONVIF.METADATA") ==
                              0  // 'ONVIF' 'metadata' (a XML document)
            ) {
            ...
            if (createSimpleRTPSource) {
                ...
                fReadSource = fRTPSource = SimpleRTPSource::createNew(
                    env(), fRTPSocket, fRTPPayloadFormat, fRTPTimestampFrequency, mimeType,
