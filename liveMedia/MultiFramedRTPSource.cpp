...
MultiFramedRTPSource ::MultiFramedRTPSource(UsageEnvironment &env, Groupsock *RTPgs,
...
void MultiFramedRTPSource::doGetNextFrame1() {
    while (fNeedDelivery) {
        ...
        nextPacket->use(fTo, fMaxSize, frameSize, fNumTruncatedBytes, fCurPacketRTPSeqNum,
                        fCurPacketRTPTimestamp, fPresentationTime,
        ...
        if (fCurrentPacketCompletesFrame && fFrameSize > 0) {
            ...
            if (fNumTruncatedBytes > 0) {
                envir() << "MultiFramedRTPSource::doGetNextFrame1(): The total received frame size "
                           "exceeds the client's buffer size ("
    ...
    do {
        ...
        if (!bPacket->fillInData(fRTPInterface, fromAddress, packetReadWasIncomplete)) {
            if (bPacket->bytesAvailable() == 0) {  // should not happen??
                envir() << "MultiFramedRTPSource internal error: Hit limit when reading incoming "
                           "packet over TCP\n";
            }
            fPacketReadInProgress = NULL;
            break;
        ...
        if (fCrypto != NULL) {  // The packet is SRTP; authenticate/decrypt it first
            ...
            if (!fCrypto->processIncomingSRTPPacket(bPacket->data(), bPacket->dataSize(),
                                                    newPacketSize))
                break;
        ...
        if (rtpPayloadType != rtpPayloadFormat()) {
            if (fRTCPInstanceForMultiplexedRTCPPackets != NULL && rtpPayloadType >= 64 &&
                rtpPayloadType <= 95) {
        ...
        Boolean usableInJitterCalculation =
            packetIsUsableInJitterCalculation((bPacket->data()), bPacket->dataSize());
        struct timeval presentationTime;  // computed by:
        Boolean hasBeenSyncedUsingRTCP;   // computed by:
        receptionStatsDB().noteIncomingPacket(rtpSSRC, rtpSeqNo, rtpTimestamp, timestampFrequency(),
...
BufferedPacket::BufferedPacket()
    : fPacketSize(MAX_PACKET_SIZE), fBuf(new unsigned char[MAX_PACKET_SIZE]), fNextPacket(NULL) {}
...
unsigned BufferedPacket ::nextEnclosedFrameSize(unsigned char *& /*framePtr*/, unsigned dataSize) {
...
void BufferedPacket ::getNextEnclosedFrameParameters(unsigned char *&framePtr, unsigned dataSize,
                                                     unsigned &frameSize,
...
Boolean BufferedPacket::fillInData(RTPInterface &rtpInterface, struct sockaddr_storage &fromAddress,
    ...
    if (!rtpInterface.handleRead(&fBuf[fTail], maxBytesToRead, numBytesRead, fromAddress,
...
void BufferedPacket ::assignMiscParams(unsigned short rtpSeqNo, unsigned rtpTimestamp,
...
void BufferedPacket::use(unsigned char *to, unsigned toSize, unsigned &bytesUsed,
                         unsigned &bytesTruncated, unsigned short &rtpSeqNo, unsigned &rtpTimestamp,
...
BufferedPacket *BufferedPacketFactory ::createNewPacket(MultiFramedRTPSource * /*ourSource*/) {
...
ReorderingPacketBuffer ::ReorderingPacketBuffer(BufferedPacketFactory *packetFactory)
    : fThresholdTime(100000) /* default reordering threshold: 100 ms */,
      ...
      fSavedPacket(NULL),
      fSavedPacketFree(True) {
    fPacketFactory = (packetFactory == NULL) ? (new BufferedPacketFactory) : packetFactory;
}
