...
OutputSocket::OutputSocket(UsageEnvironment &env, Port port)
    : Socket(env, port), fSourcePort(0), fLastSentTTL(256 /*hack: a deliberately invalid value*/) {}
...
destRecord ::destRecord(struct sockaddr_storage const &addr, Port const &port, u_int8_t ttl,
...
Groupsock::Groupsock(UsageEnvironment &env, struct sockaddr_storage const &groupAddr, Port port,
                     u_int8_t ttl)
...
destRecord *Groupsock ::createNewDestRecord(struct sockaddr_storage const &addr, Port const &port,
...
unsigned Groupsock ::lookupSessionIdFromDestination(
    struct sockaddr_storage const &destAddrAndPort) const {
...
void Groupsock::addDestination(struct sockaddr_storage const &addr, Port const &port,
    ...
    for (destRecord *dest = fDests; dest != NULL; dest = dest->fNext) {
        if (dest->fSessionId == sessionId && dest->fGroupEId.groupAddress() == addr &&
...
Boolean Groupsock::output(UsageEnvironment &env, unsigned char *buffer, unsigned bufferSize) {
    do {
        ...
        for (destRecord *dests = fDests; dests != NULL; dests = dests->fNext) {
            if (!write(dests->fGroupEId.groupAddress(), dests->fGroupEId.ttl(), buffer,
                       bufferSize)) {
                writeSuccess = False;
                break;
            }
...
Boolean Groupsock::handleRead(unsigned char *buffer, unsigned bufferMaxSize, unsigned &bytesRead,
                              struct sockaddr_storage &fromAddressAndPort) {
    bytesRead = 0;  // initially

    int numBytes = readSocket(env(), socketNum(), buffer, bufferMaxSize, fromAddressAndPort);
    if (numBytes < 0) {
        if (DebugLevel >= 0) {  // this is a fatal error
            UsageEnvironment::MsgString msg = strDup(env().getResultMsg());
...
destRecord *Groupsock ::lookupDestRecordFromDestination(
    struct sockaddr_storage const &targetAddrAndPort) const {
    for (destRecord *dest = fDests; dest != NULL; dest = dest->fNext) {
...
UsageEnvironment &operator<<(UsageEnvironment &s, const Groupsock &g) {
    UsageEnvironment &s1 = s << timestampString() << " Groupsock(" << g.socketNum() << ": "
                             << AddressString(g.groupAddress()).val() << ", " << g.port() << ", ";
    if (g.isSSM()) {
        return s1 << "SSM source: " << AddressString(g.sourceFilterAddress()).val() << ")";
    } else {
...
Groupsock *GroupsockLookupTable::Fetch(UsageEnvironment &env,
                                       struct sockaddr_storage const &groupAddress, Port port,
                                       u_int8_t ttl, Boolean &isNew) {
    isNew = False;
    Groupsock *groupsock;
    do {
...
Groupsock *GroupsockLookupTable::Lookup(struct sockaddr_storage const &groupAddress, Port port) {
...
Boolean GroupsockLookupTable::Remove(Groupsock const *groupsock) {
    ...
    return fTable.Remove(groupsock->groupAddress(), groupsock->sourceFilterAddress(),
                         groupsock->port());
}
