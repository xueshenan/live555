...
int setupDatagramSocket(UsageEnvironment &env, Port port) {
    ...
    if (setsockopt(newSocket, IPPROTO_IP, IP_MULTICAST_LOOP, (const char *)&loop, sizeof loop) <
        0) {
    ...
    if (SendingInterfaceAddr != INADDR_ANY) {
        ...
        if (setsockopt(newSocket, IPPROTO_IP, IP_MULTICAST_IF, (const char *)&addr, sizeof addr) <
            0) {
...
Boolean writeSocket(UsageEnvironment &env, int socket,
                    struct sockaddr_storage const &addressAndPort, u_int8_t ttlArg,
...
Boolean writeSocket(UsageEnvironment &env, int socket,
                    struct sockaddr_storage const &addressAndPort, unsigned char *buffer,
                    unsigned bufferSize) {
    do {
        SOCKLEN_T dest_len =
        ...
        if (bytesSent != (int)bufferSize) {
            char tmpBuf[100];
            sprintf(tmpBuf,
                    "writeSocket(%d), sendTo() error: wrote %d bytes instead of %u: ", socket,
...
static unsigned increaseBufferTo(UsageEnvironment &env, int bufOptName, int socket,
                                 unsigned requestedSize) {
...
unsigned increaseReceiveBufferTo(UsageEnvironment &env, int socket, unsigned requestedSize) {
    printf("call increaseReceiveBufferTo :%d, %d\n", socket, requestedSize);
...
Boolean socketJoinGroupSSM(UsageEnvironment &env, int socket,
    ...
    imr.imr_interface.s_addr = ReceivingInterfaceAddr;
#endif
    if (setsockopt(socket, IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP, (const char *)&imr,
...
Boolean socketLeaveGroupSSM(UsageEnvironment & /*env*/, int socket,
    ...
    imr.imr_interface.s_addr = ReceivingInterfaceAddr;
#endif
    if (setsockopt(socket, IPPROTO_IP, IP_DROP_SOURCE_MEMBERSHIP, (const char *)&imr,
                   sizeof(struct ip_mreq_source)) < 0) {
        return False;
    }
...
static Boolean badIPv4AddressForUs(ipv4AddressBits addr) {
    ...
    return (nAddr == 0x7F000001 /* 127.0.0.1 */
            || nAddr == 0 || nAddr == (ipv4AddressBits)(~0));
}
...
netAddressBits ourIPAddress(UsageEnvironment &env) {
    ...
    if (ourAddress == 0) {
        ...
        do {
            ...
            MAKE_SOCKADDR_IN(testAddressAndPort, ((struct sockaddr_in &)testAddr).sin_addr.s_addr,
                             testPort.num());
...
int gettimeofday(struct timeval *tp, int * /*tz*/) {
    ...
    tp->tv_usec =
        (long)(((tickNow.QuadPart % tickFrequency.QuadPart) * 1000000L) / tickFrequency.QuadPart);
